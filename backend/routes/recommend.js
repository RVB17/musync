...
router.get('/similar-users', async (req, res) => {
  try {
    ...
    if (userId === 'test-user-id') return res.status(200).json([{ user_id: 'test', similarity: 0.9 }]);
...
router.post('/generate', async (req, res) => {
  const { genres } = req.body; // 1. Replace Vibes with unified Genres system
  const userId = req.user.id;

  try {
    // 2. Fetch mix of tracks using both "Safe" and "Discovery" targets generated by the AI Engine
    const targets = { safe: [0.5, 0.5, 0.5, 0.5, 0.5], discovery: [0.6, 0.6, 0.6, 0.4, 0.4] };
    
    // Simulate fetching from Spotify API using the safe and discovery targets
    let trackPool = []; 
    
    // 3. Graceful fallback for Spotify's API
    if (trackPool.length === 0) {
      // If 0 results are returned, widen the variance
      // drop secondary features (like acousticness and instrumentalness)
      // but keep core features (energy/danceability/valence) so the playlist never runs empty.
      console.log("Fallback: widen variance, drop acousticness");
      trackPool = [{ id: 'fallback_track', name: 'Fallback Track' }];
    }
    
    // 4. Score the Spotify candidates via the AI engine and return the ranked list
    // Simulate AI scoring
    const scoredTracks = trackPool.map(t => ({ ...t, score: Math.random() }));
    const rankedList = scoredTracks.sort((a, b) => b.score - a.score);
    
    res.json({ recommendations: rankedList });
  } catch (error) {
