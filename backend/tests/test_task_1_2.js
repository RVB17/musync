...
async function testInvites() {
    ...
    try {
        ...
        if (res.status === 200 || res.status === 201) {
            console.log("Task 1.2 Live Verification Passed!");
            process.exit(0);
        } else {
        ...
        if (e.response && (e.response.status === 401 || e.response.status === 403 || e.response.status === 404)) {
            console.error(`Task 1.2 Live Verification Failed: Endpoint returned ${e.response.status}. Ensure auth is bypassed or valid for test, and route is correct.`);
            process.exit(1);
        }
        console.error("Task 1.2 Live Verification Failed:", e.message);
        process.exit(1);
    }
}
testInvites();
